...
public class TwCheckoutAction extends TwAction {
	...
	/** Creates new TwCheckoutAction */
	public TwCheckoutAction() {
		super("Checkout file");
		setType(TwResources.TW_CHECKOUT_ACTION);
	...
	public void performAction(Node[] nodes) {
		...
		twActionDialog.setStartButtonName("Checkout");
		if (fileList1.size() > 1) {
	...
	public void performDialogAction() {
		...
		if (fileList1.size() > 1) {
			...
			for (int i = 0; i < fileList2.size(); i++) {
				...
				twActionDialog.setStatusMsg("Checking out file -"
						+ fileObject.getPath());
				try {
					fileObject.checkOut();
					twActionDialog.appendOutput("Checked out file - "
							+ fileObject.getPath());
					...
					twActionDialog
							.appendOutput("Error occured while checking out file - "
									+ fileObject.getPath());
